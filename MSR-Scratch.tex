...
Some utilize program data collected during specific programming courses (e.g., \cite{meerbaum-salant_learning_2010, wilson_evaluation_2012, Maloney_2008}), while others utilize the dataset made available by the Lifelong Kindergarten Group at the MIT Media Lab \jemole{perhaps we could cite the dataset itself~\cite{2017_scratch_dataset}}, which contains data for Scratch projects created until 2012 (e.g., \cite{fields_2014, yang_2015, Dasgupta_2016}).
...
In addition, Dr. Scratch also detects intra-project software cloning, i.e. repetition of code; the use of custom blocks, which is the way functionality can be reused in Scratch programs; and the use of instances of sprites, a feature labeled in Scratch as clone creation. In consequence, these values are also included in the dataset.
...
\begin{table}[]
	...
	\begin{tabular}{llp{5.4cm}}
		...
		& & \texttt{Abstraction} (Dr. Scratch score for abstraction and problem decomposition)\\
		& & \texttt{Parallelism} (Dr. Scratch score for parallelism)\\
		& & \texttt{Logic} (Dr. Scratch score for logical thinking)\\
		& & \texttt{Synchronization} (Dr. Scratch score for synchronization)\\
		& & \texttt{FlowControl} (Dr. Scratch score for algorithmic notions of flow control)\\
		& & \texttt{UserInteractivity} (Dr. Scratch score for user interactivity)\\
		& & \texttt{DataRepresentation} (Dr. Scratch score for data representation)\\
		...
		& & \texttt{InstancesSprites} (True if project makes use of Scratch clones)\\
		\hline
	\end{tabular}
...
\jemole{I don't know if we have space for this, but here it goes a proposal: As shown in Table~\ref{tbl-size}, not all the projects were successfully analyzed with Dr. Scratch. This is due to problems with non-official extension blocks and with some non-ascii characters in sprites names. Of those correctly assessed projects, as depicted in Table~\ref{tbl-stats}, the mastery score of the median project is 8, which is considered a medium level of development of computational thinking skills.}
